...
dfr.set_index("age span").plot(kind="bar", color=["red", "steelblue"], alpha=0.5)
...
df.groupby("position")["gender"].hist(legend=True, alpha=0.5)
plt.title("Gender distribution per political orientation")
plt.tight_layout()
plt.savefig("../charts/how_meaningful_is_age/position_gender_hist.png")
plt.clf()

df.groupby("party")["gender"].hist(legend=True, alpha=0.5)
plt.title("Gender distribution per political party")
plt.tight_layout()
plt.savefig("../charts/how_meaningful_is_age/party_gender_hist.png")
plt.clf()
